    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex h-full flex-1 flex-col overflow-y-auto">
            ...
            <header class="flex h-16 items-center justify-between border-b border-gray-200 bg-white px-6 dark:bg-gray-800 dark:border-gray-700">
                ...
                <h1 class="text-xl font-bold text-gray-800 dark:text-white hidden md:block">Data Pendaftar Mahasiswa Baru</h1>
                    ...
                    <div class="relative ml-2">
                        ...
                        <div id="admin-user-dropdown" class="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 dark:bg-gray-800 hidden z-50">
                            ...
                            <ul class="py-1">
                                ...
                                <li><form method="POST" action="{{ route('logout') }}">@csrf<button type="submit" class="block w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20">Log Out</button></form></li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <main class="flex-1 p-6">
                @if (session('success'))
                    <div id="alert-success" class="flex items-center justify-between p-4 mb-4 text-green-800 rounded-lg bg-green-100 dark:bg-green-900/50 dark:text-green-300" role="alert">
                        ...
                        <button onclick="document.getElementById('alert-success').style.display='none'" class="text-green-600 hover:bg-green-200 p-1 rounded">x</button>
                    </div>
                @endif

                <form method="GET" action="{{ route('admin.pendaftar') }}" class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                    <div class="flex flex-col sm:flex-row gap-4 w-full">
                        <div class="relative w-full sm:w-64">
                            <input type="text" name="search" value="{{ request('search') }}" autocomplete="off" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 text-sm focus:border-campus-600 focus:ring-campus-600 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Cari nama / email...">
                        </div>
                        <select name="prodi" onchange="this.form.submit()" class="block w-full sm:w-48 rounded-lg border border-gray-300 bg-gray-50 p-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        ...
                        <select name="status" onchange="this.form.submit()" class="block w-full sm:w-48 rounded-lg border border-gray-300 bg-gray-50 p-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                    ...
                                    <tr class="bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <img class="h-6 w-6 rounded-full object-cover" src="{{ $item->user && $item->user->avatar ? asset('storage/' . $item->user->avatar) : asset('images/user.png') }}" alt="Avatar">
                                        ...
                                        <td class="px-6 py-4">
                                            @if($item->status == 'pending')
                                                <span class="inline-flex items-center rounded-md bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300">Menunggu Verifikasi Biodata</span>
                                            @elseif($item->status == 'verified')
                                                @if($item->pembayaran)
                                                    @if($item->pembayaran->status == 'pending')
                                                        <span class="inline-flex items-center rounded-md bg-blue-100 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-300">Cek Pembayaran</span>
                                                    @elseif($item->pembayaran->status == 'valid')
                                                        <span class="inline-flex items-center rounded-md bg-green-100 px-2 py-1 text-xs font-medium text-green-700 dark:bg-green-900 dark:text-green-300">Selesai / Lulus</span>
                                                    @elseif($item->pembayaran->status == 'invalid')
                                                        <span class="inline-flex items-center rounded-md bg-red-100 px-2 py-1 text-xs font-medium text-red-700 dark:bg-red-900 dark:text-red-300">Pembayaran Ditolak</span>
                                                    @endif
                                                @else
                                            ...
                                            @elseif($item->status == 'rejected')
                                                <span class="inline-flex items-center rounded-md bg-red-100 px-2 py-1 text-xs font-medium text-red-700 dark:bg-red-900 dark:text-red-300">Ditolak</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <a href="{{ route('admin.pendaftar.show', $item->id) }}" class="font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors" title="Lihat Detail">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                </a>

                                                <a href="{{ route('admin.pendaftar.edit', $item->id) }}" class="font-medium text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-300 transition-colors" title="Edit Data Pendaftar">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                </a>

                                                @if($item->status == 'pending')
                                                    <button type="button" onclick="openVerificationModal('{{ route('admin.pendaftar.verify', ['id' => $item->id, 'status' => 'verified']) }}', 'verified', '{{ $item->name }}')" class="font-medium text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 transition-colors" title="Terima Biodata">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                                    </button>
                                                    <button type="button" onclick="openVerificationModal('{{ route('admin.pendaftar.verify', ['id' => $item->id, 'status' => 'rejected']) }}', 'rejected', '{{ $item->name }}')" class="font-medium text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition-colors" title="Tolak Biodata">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                    </button>
                                                @endif

                                                @if($item->status == 'verified' && $item->pembayaran && $item->pembayaran->status == 'pending')
                                                    <a href="{{ route('admin.pendaftar.show_payment', $item->id) }}" class="font-medium text-orange-600 hover:text-orange-800 dark:text-orange-400 dark:hover:text-orange-300 transition-colors" title="Verifikasi Pembayaran">
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg dark:bg-gray-800">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 dark:bg-gray-700/50">
                        ...
                        <button type="button" onclick="closeVerificationModal()" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto dark:bg-gray-700 dark:text-gray-300 dark:ring-gray-600">Batal</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function openVerificationModal(url, type, name) {
            ...
            if(type === 'verified') {
                ...
                icon.className = "mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-green-100 sm:mx-0 sm:h-10 sm:w-10";
                ...
                icon.className = "mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10";
